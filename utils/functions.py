...
def read_instance_with_gaz(input_file, gaz, word_alphabet, biword_alphabet, char_alphabet, gaz_alphabet, label_alphabet, number_normalized, max_sent_length, char_padding_size=-1, char_padding_symbol = '</pad>'):
    ...
    words = []
    biwords = []
    chars = []   #每个词的charlist的list
    labels = []
    word_Ids = []
    biword_Ids = []
    char_Ids = []   #每个词的char id list的list
    label_Ids = []
    for idx in range(len(in_lines)):
        line = in_lines[idx]
        if len(line) > 2:   #在一个句子内一直会有len>2
            pairs = line.strip().split()
            word = pairs[0]
            if number_normalized:
                word = normalize_word(word)
            ...
            for char in word:
                char_list.append(char)
            if char_padding_size > 0:   #当前词的字数不足的话用padding
                char_number = len(char_list)
                if char_number < char_padding_size:
                    char_list = char_list + [char_padding_symbol]*(char_padding_size-char_number)
                assert(len(char_list) == char_padding_size)
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words)>0):  #只保留maxlen以内的句子
                gazs = []
                gaz_Ids = []
                w_length = len(words)
                # print sentence 
                ...
                #     print w," ",
                # print
                for idx in range(w_length):   #统计这个句子中的以每个字开头能连成的词
                    matched_list = gaz.enumerateMatchList(words[idx:])
                    matched_length = [len(a) for a in matched_list]   #这个应该是用来表示文章里那个w(i,j)以句子中wi开头以wj结尾的词
                    # print idx,"----------"
                    ...
                    if matched_Id:
                        ...
                        gaz_Ids.append([])   #一定得append一个空列表，因为这个gaz_Ids的index表示是该句中第几个字开始的词
...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True):    
    ...
    perfect_match = 0
    case_match = 0   #中文这里case和perfect一样，又没有大小写
    not_match = 0
...
# 用于position embedding
def build_embedding(embedding_path, word_alphabet_size, embedd_dim=100):
    embedd_dict = dict()
    if embedding_path != None:
        embedd_dict, embedd_dim = load_pretrain_emb(embedding_path)

    scale = np.sqrt(3.0 / embedd_dim)
    pretrain_emb = np.empty([word_alphabet_size, embedd_dim])
    perfect_match = 0
    case_match = 0   #中文这里case和perfect一样，又没有大小写
    not_match = 0
    for index in range(word_alphabet_size): #随机生成
        pretrain_emb[index,:] = np.random.uniform(-scale, scale, [1, embedd_dim])
        not_match += 1
    pretrained_size = len(embedd_dict)
    print("Embedding:\n     pretrain word:%s, prefect match:%s, case_match:%s, oov:%s, oov%%:%s"%(pretrained_size, perfect_match, case_match, not_match, (not_match+0.)/word_alphabet_size))
    return pretrain_emb, embedd_dim
